...
8. [CLI + `create-mantlejs` verification](#8-cli--create-mantlejs-verification)
...
## 8. CLI + `create-mantlejs` verification
...
- **Smoke test:** an Nx target (`e2e-scaffold`) run in CI: `create-mantlejs` into a temp dir (non-interactive
...
- **Post-release:** smoke-install loop (`npm install` each package into a temp project, import it, assert the
  main export), `npm create mantlejs` live run, re-point `todo-minimal` at registry versions and boot, tag
