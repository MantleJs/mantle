...
| 3 | `@mantlejs/audit` and `@mantlejs/embeddings` default to `0.2.0-experimental` (not the literal `0.1.0-experimental` this row originally specified) at first release | Standing rule from Phase 5 (Decision #11): new packages don't go straight to stable in their introduction phase, absent a flagship-differentiator exception like `mcp`'s. Revisit at the Phase 6 stage-2 tier-list review if evidence argues otherwise, same process Phase 5 used for `openapi`. **Amended during item 9 execution**: the literal `0.1.0-experimental` figure was inherited from Phase 5's precedent without re-deriving it — Phase 5 used it only because that was the project's first-ever release, so `0.1.0` was the obvious starting point for both tiers simultaneously at the same time. It doesn't generalize: `nx.json`'s `releaseTagPattern` is `v{version}`, not group-aware, and the `experimental` group's membership isn't permanent (Decision #8 — Phase 5's `experimental` group emptied via promotion and was removed; this phase re-added it from scratch for two unrelated packages). Reusing the literal string `0.1.0-experimental` for a second, unrelated group's first release collided with the git tag Phase 5's `experimental` group already used for its own first release (confirmed by attempting it — `nx release version 0.1.0-experimental --groups=experimental --first-release` succeeded, but `git tag v0.1.0-experimental` was already taken). Fixed by tying a new experimental package's first-release version to the *current stable release cycle's* version instead of a hardcoded `0.1.0` — `stable` is bumping to `0.2.0` in this same item, so `audit`/`embeddings` start at `0.2.0-experimental`. This can't collide with `stable`'s own tags (different suffix) and won't recur the way the hardcoded literal did, since `stable`'s version only ever climbs — a future experimental-group refill will pick up whatever `stable`'s version is at that time, never a repeated literal. |
...
| 9 | Spike finding (2026-09-22, item 6/spec 12 prerequisite): `@mantlejs/pinecone`, `@mantlejs/qdrant`, and `@mantlejs/mongodb`'s `MongoVectorRepository` all require the caller to supply an already-computed `number[]` vector — none generates an embedding from source text internally. Confirmed by reading `upsertVector`/`findSimilar` in all three (`pinecone-repository.ts`, `qdrant-repository.ts`, `mongo-vector-repository.ts` — each takes `vector: number[]` and passes it straight to the backend's native API/aggregation stage), each package's own README stating this design choice explicitly ("model-agnostic", "embedding generation intentionally decoupled"), and `save()`/`saveAll()` on `pinecone`/`qdrant` writing an explicit zero-vector placeholder rather than a real embedding. `examples/knowledge-base` already hand-rolls the gap itself: `api/src/embedder.ts`'s `Embedder` interface (`localEmbedder` hash-based demo default, or `httpEmbedder` delegating to an external HTTP endpoint) invoked manually by `ArticlesService.reembed()` in `api/src/services/articles-service.ts`. | This is the expected outcome the PRD called out in advance (spec 12) — no adapter conformance gap results (all three already agree), and `@mantlejs/embeddings` is confirmed net-new capability, not a duplicate of something an adapter already does. Building it now proceeds per spec 12's plan; the concrete replacement target in `examples/knowledge-base` is `ArticlesService.reembed()` |
