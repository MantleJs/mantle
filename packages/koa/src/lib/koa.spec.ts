...
describe("koa adapter", () => {
  describe("plugin setup", () => {
    it("returns a MantlePlugin function", () => {
      expect(typeof koa()).toBe("function");
    });

    it("stores a Koa application on app.get('koa')", () => {
      const app = mantle().configure(koa());
      expect(app.get("koa")).toBeDefined();
      expect(app.get("koa")).toBeInstanceOf(KoaLib);
    });

    it("accepts an existing Koa instance", () => {
  ...
  describe("params population", () => {
    it("sets params.provider to 'koa' for HTTP requests", async () => {
      ...
      try {
        await fetch(`http://localhost:${port}/users`);
        expect(capturedProvider).toBe("koa");
      } finally {
        await stop();
      }
    });
  ...
  describe("error handling", () => {
    it("serialises NotFound (404) from a service to a 404 HTTP response", async () => {
  ...
  describe("method restriction", () => {
    it("returns 405 when a non-registered method is called on a known route", async () => {
      const { port, stop } = await startApp((app) => {
      ...
      try {
        ...
        // @koa/router allowedMethods() returns 405 (more correct than Express's 404)
...
describe("POST /batch", () => {
  async function startWith(options?: Parameters<typeof koa>[0]): Promise<{
